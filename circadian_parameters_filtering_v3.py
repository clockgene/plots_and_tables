...
"""
# adding row names to oscillatory_params.csv

data = pd.read_csv(glob.glob(f'{mydir}*oscillatory_params.csv')[0])
data.loc[data.Y == 1.0, 'row'] = 'A'
data.loc[data.Y == 2.0, 'row'] = 'B'
data.loc[data.Y == 3.0, 'row'] = 'C'
data.loc[data.Y == 4.0, 'row'] = 'D'
data.loc[data.Y == 5.0, 'row'] = 'E'
data.loc[data.Y == 6.0, 'row'] = 'F'
data.loc[data.Y == 7.0, 'row'] = 'G'
data.loc[data.Y == 8.0, 'row'] = 'H'
data.loc[data.Y == 9.0, 'row'] = 'I'
data.loc[data.Y == 10.0, 'row'] = 'J'
data.loc[data.Y == 11.0, 'row'] = 'K'
data.loc[data.Y == 12.0, 'row'] = 'L'
data.loc[data.Y == 13.0, 'row'] = 'M'
data.loc[data.Y == 14.0, 'row'] = 'N'
data.loc[data.Y == 15.0, 'row'] = 'O'
data.loc[data.Y == 16.0, 'row'] = 'P'
data['column'] = data.X
data.to_csv(f'ex67_oscillatory_params.csv', index=False)
...
# CIRCADIAN PARAMETERS filtering to remove noise from LV200 data - NOT FINISHED, use Composite P2p v7 instead
"""
data = pd.read_csv('parameters.csv')

all_cols = ['Period_CHP1', 'Period_CHP2', 'Period_CHP3', 'Period_CHP4',
       'Period_CHP5', 'Period_CHP6', 'Amplitude_CHP1', 'Amplitude_CHP2',
       'Amplitude_CHP3', 'Amplitude_CHP4', 'Amplitude_CHP5',
       'Amplitude_CHP6', 'Phase_CHP1', 'Phase_CHP2', 'Phase_CHP3',
       'Phase_CHP4', 'Phase_CHP5', 'Phase_CHP6', 'Trend_CHP1',
       'Trend_CHP2', 'Trend_CHP3', 'Trend_CHP4', 'Trend_CHP5',
       'Trend_CHP6', 'Decay_CHP1', 'Decay_CHP2', 'Decay_CHP3',
       'Decay_CHP4', 'Decay_CHP5', 'Decay_CHP6', 'Rsq_CHP1', 'Rsq_CHP2',
       'Rsq_CHP3', 'Rsq_CHP4', 'Rsq_CHP5', 'Rsq_CHP6']

data.loc[(data['Trend_CHP1'] > data['Trend_CHP1'].quantile(q=0.25)), 'Period_CHP1'].mean()
