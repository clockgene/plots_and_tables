...
# Was there treatment? Yes = 1, No = 0
treat = 0

# Choose to plot heatmap of K, Halflife, Trend, Phases, Amplitudes or Periods, or Phases_Histogram
graphtype = 'Phases_Histogram'

# set number of rows, i.e. how many individual SCNs were analyzed. For treat = 0, edit Nc and Nw variables as well, typically 2 cols x 3 rows.
Nr = 6
...
if graphtype == 'Phases' or graphtype == 'Phases_Histogram':
    cmap = mpl.colors.ListedColormap(circular_colors)
if graphtype == 'Amplitudes':
    cmap="YlGnBu"
if graphtype == 'Periods':
    cmap="plasma"
...
if treat == 1:
    ...
    if graphtype == 'Phases' or graphtype == 'Amplitudes' or graphtype == 'Periods':
        ...
        for i in range(Nr):
            for j in range(1, -1, -1):
                ...
                if graphtype == 'Amplitudes':           
                    ...
                    data_filtered = reject_outliers(data_a)
                    checkdata = checkdata.append(data_filtered)
        
                    # need to normalize amplitude for common colormap
                    data_filtered.iloc[:, 2] = data_filtered.iloc[:, 2] / data_filtered.iloc[:, 2].max() * 100
                    
                    # pivot and transpose for heatmap format
                    df_heat = data_filtered.pivot(index='X', columns='Y', values='Amplitude').transpose()
                ...
                if graphtype == 'Periods':
                    ...
                    data_filtered = reject_outliers(data_p, column='Period')
                    # pivot and transpose for heatmap format
                    df_heat = data_filtered.pivot(index='X', columns='Y', values='Period').transpose()
        ...
        if graphtype == 'Phases':
            ...
            cbar.ax.set_ylabel('Phase (h)', rotation=270, fontsize=5, labelpad=5)
            cbar.ax.set_yticklabels((3, 6, 9, 12, 15, 18, 21), fontsize=5)
            ...
            #fig.tight_layout()
            ### To save as bitmap png for easy viewing ###
            plt.savefig(f'{path}Composite_Heatmap_XY_Phase.png', dpi=1000)
            ### To save as vector svg with fonts editable in Corel ###
            plt.savefig(f'{path}Composite_Heatmap_XY_Phase.svg', format = 'svg') #if using rasterized = True to reduce size, set-> dpi = 1000
            plt.clf()
            plt.close()
        ...
        if graphtype == 'Amplitudes':
            ...
            #fig.tight_layout()
            ### To save as bitmap png for easy viewing ###
            plt.savefig(f'{path}Composite_Heatmap_XY_Amp.png', dpi=1000)
            ### To save as vector svg with fonts editable in Corel ###
            plt.savefig(f'{path}Composite_Heatmap_XY_Amp.svg', format = 'svg') #if using rasterized = True to reduce size, set-> dpi = 1000
            plt.clf()
            plt.close()
        ...
        if graphtype == 'Periods':
            ...
            cbar = fig.colorbar(images[0], ax=axs, orientation='vertical', fraction=.1)
            cbar.ax.set_ylabel('Period (h)', rotation=270, labelpad=11)
            ...
            #fig.tight_layout()
            ### To save as bitmap png for easy viewing ###
            plt.savefig(f'{path}Composite_Heatmap_XY_Period.png', dpi=1000)
            ### To save as vector svg with fonts editable in Corel ###
            plt.savefig(f'{path}Composite_Heatmap_XY_Period.svg', format = 'svg') #if using rasterized = True to reduce size, set-> dpi = 1000
            plt.clf()
            plt.close()
    ...
    if graphtype == 'Halflife':
        ...
        for i in range(Nw):
            for j in range(Nc):
                ...
                data_h = np.round(data_hl[['X', 'Y', 'Halflife']], decimals=2)
        ...
        cbar = fig.colorbar(images[0], ax=axs, orientation='vertical', fraction=.1)
        cbar.ax.set_ylabel('Half-life (h)', rotation=270, labelpad=11)
        ...
        #fig.tight_layout()
        ### To save as bitmap png for easy viewing ###
        plt.savefig(f'{path}Composite_Heatmap_XY_Halflife.png', dpi=1000)
        ### To save as vector svg with fonts editable in Corel ###
        plt.savefig(f'{path}Composite_Heatmap_XY_Halflife.svg', format = 'svg') #if using rasterized = True to reduce size, set-> dpi = 1000
        plt.clf()
        plt.close()
    ...
    if graphtype == 'K':
        ...
        for i in range(Nw):
            for j in range(Nc):
                ...
                data_h = np.round(data_hl[['X', 'Y', 'K']], decimals=3)
                #data_filtered = reject_outliers(data_h, column='Halflife')
                # pivot and transpose for heatmap format
                df_heat = data_h.pivot(index='X', columns='Y', values='K').transpose()
        ...
        cbar = fig.colorbar(images[0], ax=axs, orientation='vertical', fraction=.1)
        cbar.ax.set_ylabel('Degradation constant', rotation=270, labelpad=11)
        ...
        #fig.tight_layout()
        ### To save as bitmap png for easy viewing ###
        plt.savefig(f'{path}Composite_Heatmap_XY_K.png', dpi=1000)
        ### To save as vector svg with fonts editable in Corel ###
        plt.savefig(f'{path}Composite_Heatmap_XY_K.svg', format = 'svg') #if using rasterized = True to reduce size, set-> dpi = 1000
        plt.clf()
        plt.close() 
    ...
    if graphtype == 'Trend':
        ...
        for i in range(Nw):
            for j in range(Nc):
                ...
                data_h = np.round(data_hl[['X', 'Y', 'Trend']], decimals=2)
                #data_filtered = reject_outliers(data_h, column='Halflife')
                # pivot and transpose for heatmap format
                df_heat = data_h.pivot(index='X', columns='Y', values='Trend').transpose()
        ...
        cbar = fig.colorbar(images[0], ax=axs, orientation='vertical', fraction=.1)
        cbar.ax.set_ylabel('PER2 Expression level', rotation=270, labelpad=11)
        ...
        #fig.tight_layout()
        ### To save as bitmap png for easy viewing ###
        plt.savefig(f'{path}Composite_Heatmap_XY_Trend.png', dpi=1000)
        ### To save as vector svg with fonts editable in Corel ###
        plt.savefig(f'{path}Composite_Heatmap_XY_Trend.svg', format = 'svg') #if using rasterized = True to reduce size, set-> dpi = 1000
        plt.clf()
        plt.close() 
    ...
    if graphtype == 'Amplitudes':
        ...
        for i in range(Nw):
            for j in range(Nc):
                ...
                #data_filtered = reject_outliers(data_h, column='Halflife')
                # pivot and transpose for heatmap format
                df_heat = data_h.pivot(index='X', columns='Y', values='Amplitude').transpose()
        ...
        cbar = fig.colorbar(images[0], ax=axs, orientation='vertical', fraction=.1)
        cbar.ax.set_ylabel('Amplitude', rotation=270, labelpad=11)
        ...
        #fig.tight_layout()
        ### To save as bitmap png for easy viewing ###
        plt.savefig(f'{path}Composite_Heatmap_XY_Amplitude.png', dpi=1000)
        ### To save as vector svg with fonts editable in Corel ###
        plt.savefig(f'{path}Composite_Heatmap_XY_Amplitude.svg', format = 'svg') #if using rasterized = True to reduce size, set-> dpi = 1000
        plt.clf()
        plt.close()  
    ...
    if graphtype == 'Periods':
        ...
        for i in range(Nw):
            for j in range(Nc):
                ...
                data_h = np.round(data[['X', 'Y', 'Period']], decimals=2)
                #data_filtered = reject_outliers(data_h, column='Halflife')
                # pivot and transpose for heatmap format
                df_heat = data_h.pivot(index='X', columns='Y', values='Period').transpose()
    ...
    if graphtype == 'Phases':
        ...
        for i in range(Nw):
            for j in range(Nc):
                ...
                data_h = np.round(data[['X', 'Y', 'Phase']], decimals=2)
                #data_filtered = reject_outliers(data_h, column='Halflife')
                # pivot and transpose for heatmap format
                df_heat = data_h.pivot(index='X', columns='Y', values='Phase').transpose()
