...
df_cos_max['Phase_final'] = df_cos_max.apply(lambda x: phase_correct_2(x['Phase'], x['Phase_12'], x['CT']), axis=1)     #apply Phase correcting function to try remediate FFT Cosinor problem
# Final Phase is still sometimes wrong, just use Peaks and Phase_SD for plotting and as Phase
...
data = df_cos_max.copy().reset_index().set_index(['tissue', 'food', 'function']).sort_values(by=['tissue', 'food', 'function'])    #this contains multiindexed dataframe with final correct phase

#Use CTRL+H to replace tissue name to create different plots

phaseseries = data.loc[('liv', 'LD'), 'Peaks'][((data.loc[('liv', 'LD'), 'P_value']) < 0.05) | ((data.loc[('liv', 'RF'), 'P_value']) < 0.05)].values.flatten()   # only include those with P<0.05 in LD or RF
...
####### Data RF ##########
phaseseries1 = data.loc[('liv', 'RF'), 'Peaks'][((data.loc[('liv', 'LD'), 'P_value']) < 0.05) | ((data.loc[('liv', 'RF'), 'P_value']) < 0.05)].values.flatten()
